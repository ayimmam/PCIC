...
app.use(cors({
  origin: ["https://pcic.tech", "http://localhost:5173"],
...
// Catch-all for undefined API routes to distinguish from Vercel 404s
app.use("/api/*", (req, res) => {
  console.log(`404 at ${req.originalUrl} - ${req.method}`);
  res.status(404).json({ error: "Route not found in Express", path: req.originalUrl });
});
